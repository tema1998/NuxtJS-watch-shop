<template>
        ...
        <nav class="flex justify-center mt-5" aria-label="Page navigation example">
          <ul class="flex items-center -space-x-px h-10 text-base">
            ...
            <span v-for="i in total">
              <li v-if="current_page === i || ($route.fullPath === `/search?search=${search_query}` && i === 1)">
                <nuxt-link :to="`?page=${i}&search=${search_query}`" aria-current="page" class="z-10 flex items-center justify-center px-4 h-10 leading-tight text-blue-600 border border-blue-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{i}}</nuxt-link>
              </li>

              <li v-else>
                <nuxt-link :to="`?page=${i}&search=${search_query}`" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{{i}}</nuxt-link>
              </li>
            </span>

            <li v-if="next != null">
              <nuxt-link :to="next" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
  ...
  export default {
    ...
    async asyncData({route}) {
      ...
      let search_query = route.query.search;
      return {
